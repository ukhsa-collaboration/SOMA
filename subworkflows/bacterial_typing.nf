...
workflow BACTERIAL_TYPING {
    ...
    ch_salmonella = ch_merged.filter({meta, fasta, fastq -> meta.genus.toLowerCase().matches("^salmonella")})
    ch_ecoli = ch_merged.filter({meta, fasta, fastq -> meta.clean_id.toLowerCase().matches("^escherichia coli")})
    ch_lmonocytogenes = ch_merged.filter({meta, fasta, fastq -> meta.clean_id.toLowerCase().matches("^listeria monocytogenes")})
