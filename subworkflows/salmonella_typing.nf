...
workflow SALMONELLA_TYPING {
    ...
    ch_salmonella_assembly = ch_salmonella.map{meta -> meta = [meta[0],meta[1]]}

    // Serotype prediction
    SEQSERO2(ch_salmonella_assembly)
    ch_versions = ch_versions.mix(SEQSERO2.out.versions)

    // Serovar prediction, identification of antigen and cgMLST genes 
    SISTR(ch_salmonella_assembly)
    ch_versions = ch_versions.mix(SISTR.out.versions)

    ch_prefilt_salmonella1 = ch_salmonella_assembly.join(SEQSERO2.out.tsv, by: [0])
